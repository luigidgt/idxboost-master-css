(function(){
  ...
  $ventana.on('load', function() {
    ...
    //23/05/2017
    var $noScroll = $('main').attr('data-scroll');
  ...
  if ($resultSearch.length) {
    ...
    //KITE AKI
    /*$resultSearch.on('click', '.btn-check', function(e) {
      var sysid = $(this).data('id');
      var type = $(this).data('type');
      var btn = $(this).children('span');
      if(parseInt(dgtCredential.anonymous)){
        $('.login a.show-modal').trigger('click');
        return false;
      } else {
        ...
        if (!$elCheck.hasClass('active')) {
          ...
          $.ajax({
            ...
            type: "POST",
            data: {action: 'dgt_favorite_fn', sysid: sysid, process:'add', post_type:type },
            dataType: "json",
            success: function() {
              btn.addClass('active');
            },
          ...
          $.ajax({
            url: dgtCredential.ajaxUrl,
            type: "POST",
            data: {action: 'dgt_favorite_fn', sysid: sysid, process:'remove', post_type:type },
            dataType: "json",
            success: function(){
      ...
      if (!$ulSliderb.hasClass('swiping')) {
        ...
        if ($marginLeftUl == '0px') {
          ...
          if ($ulSliderb.hasClass('created')) {
            ...
            var $newImages = '';
            var $mode = $wrapSlider.parent().data('mode');
  ...
  dgt_rangeSliderSnap('#range-baths', 0, 12);
  dgt_rangeSliderSnap('#range-beds', 0, 6);
  ...
  $(".header-tab a").click(function() {
    ...
    switch(tabId){
      case 'tabLogin': $('#modal_login h2').text('Welcome'); $(".text-slogan").text('Sign in below'); break;
  ...
  var $openCloseMap = $('.map-actions-1 button');
  ...
  $(document).on('click', '.show-modal', function() {
    ...
    if( $modal.hasClass('active_modal') ){
      ...
      // Get the current scroll top and saved in the body
...
//if(typeof($elementx) != 'undefined'){
  ...
  /************ FUNCION QUE CARGA EL FULL SLIDER ***************/
  //FULL SLIDER
  var $ventana = $(window);
  var $widthVentana = $ventana.width();
  function anchoRelativoSlider(wrapper, frames) {
    var nframex = frames;
    if(typeof(nframex) != 'undefined'){
      var nframes = nframex.length;
  ...
  $('body').on('touchmove', '#full-slider .wrap-slider', function(evt) {
    ...
    if ( Math.abs( xDiff ) > Math.abs( yDiff ) ) { // si se mueve derecha o izquierda
      evt.preventDefault();
      if ( xDiff > 0 ) { // izquierda
        $('#full-slider .wrap-slider .next').trigger('click');
      } else { // derecha
        $('#full-slider .wrap-slider .prev').trigger('click'); 
      }
    }
    xDown = null;
    yDown = null;
  ...
  /** AGREGADOS EL 07/06/2017 **/

  //ADAPTAR LA IMAGEN DEL MODAL DE LOGIN Y REGISTRO AL HACER CLICK EN REGISTER POR PRIMERA VEZ
  $("#user-options .register a").click(function() {
    $("#modal_login h2").text('Register'); 
    $(".text-slogan").text('Join to save listings and receive updates');
    $("#tabRegister").addClass('active');
    setTimeout(function(){
      var imgheight = $("#content-info-modal").height();
      $(".img_modal").css({'height':imgheight+'px'});
    }, 100);
  });

  //ACTIVAR BOTON DE TAB EN DETALLE DE AGENTE
  $(document).on('click', '.list-tab button', function() {
    $('.list-tab button').removeClass('active');
    $(this).addClass('active');
  });

  //ACTIVAR LOS FILTROS DE LUXURY CONDO
