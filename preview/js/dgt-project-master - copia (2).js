(function($) {
  ...
  function clidxboostSlider(slider){
    ...
    if ($sliderMain.length) {
      ...
      if($showItemSlide > 1){ $sliderMain.addClass('clidxboost-padding') }
      

      //Asignamos el ancho que tomará el item del slider donde: ((100/$showItemSlide)/100) representa el % de varianza entre cada item en relación al total de items a mostrar
      //((100 / $totalLis) - 1) se le resta 1 al total ya que 1 representa el 1% que se le esta asignando como margen
      ...
      creaBullets($bulletsShow, $contentBullets); //Creamos los bullets iniciales
      ...
      $contentBullets.on('click', 'button', function(){
        if (!$ulSlider.hasClass('swiping')) {
          ...
          if ($ventana.width() >= 768) {
            apareceImagen($liSlider.eq($indexButton * 2)); // por EQ de Jquery se trata menos 1
            apareceImagen($liSlider.eq(($indexButton * 2) + 1));
          } else {
            apareceImagen($liSlider.eq($indexButton));
            $liSlider.eq($indexButton).addClass('active').siblings().removeClass('active');
          }
      ...
      $bulletsSliderMain.on('click', function() {
        if (!$ulSlider.hasClass('swiping')) {
          ...
          var $indexButton = $(this).index();
          $ulSlider.css('margin-left', '-' + ($indexButton * 100) + '%');
          apareceImagen($liSlider.eq($indexButton), $ulSlider);
          $liSlider.eq($indexButton).addClass('active').removeClass('active');
